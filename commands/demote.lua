...
return function (rbx, client, message, args)
    ...
    if args[2] == nil then message.channel:send(embed.new("Give a **username** to demote them!")) return end
    ...
    if TargetUser.id == User.id then message.channel:send(embed.new("You cannot demote yourself!")) return end
    ...
    if SetRank > UserRole.rank then message.channel:send(embed.new("You cannot demote someone to a rank higher than your own!")) return end

    if setRank == UserRole.rank then message.channel:send(embed.new("You cannot demote someone to your own rank!")) return end
